...
import { useRouter } from "@/i18n/navigation"; // <-- Добавлен импорт роутера
...
export default function Card({ embedded = false }: CardProps) {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setIsSubmitting(false); // Выключаем загрузку только при ошибке, так как при успехе мы уходим со страницы
    }
  };
      ...
      >
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-2 lg:gap-8">
          ...
          <form className="rounded-3xl p-1 lg:p-0" onSubmit={handleSubmit}>
            ...
            <div className="flex flex-col items-center justify-between gap-4 sm:flex-row sm:flex-wrap sm:gap-6 mt-4">
              <button
                type="submit"
              ...
              <div className="flex items-center justify-center gap-3">
                <Link href="https://t.me/lazzurr" aria-label={t("social.telegram")} className="grid place-items-center">
                  <Image
                    src={socialIconSrc(isDarkTheme, "telegram")}
